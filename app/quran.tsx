...
import { HIZB_STARTS } from '@/lib/quranHizbIndex';
import { useQuranWebOffline } from '@/lib/quranWebOffline';
...
const mushafHtml = (uri: string, zoom: number) => `<html><head><meta name="viewport" content="width=device-width,initial-scale=1"></head><body style="margin:0;background:white"><img src="${uri}" style="width:${zoom * 100}%;height:auto"/><script>let startX=null;document.addEventListener('touchstart',e=>{startX=e.changedTouches[0].screenX},{passive:true});document.addEventListener('touchend',e=>{if(startX===null)return;const delta=e.changedTouches[0].screenX-startX;startX=null;if(Math.abs(delta)>75)window.ReactNativeWebView.postMessage(delta>0?'next':'prev')},{passive:true});</script></body></html>`;
...
export default function QuranScreen() {
  ...
  return <View style={styles.root}>
    ...
    {!reading ? <ScrollView contentContainerStyle={styles.dashboard}>
      ...
      {offline.supported && <View style={styles.card}><Text style={styles.heading}>{label('القراءة دون اتصال', 'Offline reading')}</Text><Text style={styles.sub}>{offline.ready ? label('المصحف كامل محفوظ على هذا الجهاز.', 'The full Mushaf is saved on this device.') : label(`تم تنزيل ${offline.readyCount} من ${offline.total} أجزاء. يلزم تنزيلها كلها قبل القراءة بلا إنترنت.`, `${offline.readyCount} of ${offline.total} parts downloaded. All parts are needed before offline reading.`)}</Text>{!offline.ready && <Text style={styles.sub}>{label('حجم التنزيل نحو 100 ميجابايت؛ يُفضَّل استخدام Wi‑Fi.', 'Download size is about 100 MB; Wi‑Fi is recommended.')}</Text>}{offline.error && <Text style={styles.warning}>{label('تعذر إكمال التنزيل. تحقق من المساحة والاتصال ثم حاول مجددًا.', 'Download failed. Check storage and connection, then retry.')}</Text>}{!offline.ready && button(offline.downloading ? label('جاري التنزيل…', 'Downloading…') : label('تنزيل المصحف كاملاً', 'Download complete Mushaf'), () => { if (!offline.downloading) void offline.download(); }, true)}</View>}
      ...
      {button(label('فهرس الأحزاب', 'Hizb index'), () => setShowIndex(showIndex === 'hizb' ? null : 'hizb'))}
      {showIndex === 'hizb' && <View style={styles.card}>{HIZB_STARTS.map(entry => <TouchableOpacity key={entry.h} style={styles.row} onPress={() => go(entry.p)}><Text>{label('الحزب', 'Hizb')} {entry.h} · {ar ? SURAHS[entry.s - 1].a : SURAHS[entry.s - 1].e} {entry.s}:{entry.a} · {label('صفحة', 'Page')} {entry.p}</Text></TouchableOpacity>)}</View>}
      ...
      <View style={styles.page} onTouchStart={event => { if (Platform.OS === 'web') touchX.current = event.nativeEvent.pageX; }} onTouchEnd={event => { if (Platform.OS !== 'web' || touchX.current == null) return; const delta = event.nativeEvent.pageX - touchX.current; touchX.current = null; if (Math.abs(delta) > 75) go(data.page + (delta > 0 ? 1 : -1)); }}>{!pageUri && !pageError && <ActivityIndicator color="#D76743" />}{pageError && button(label('تعذر فتح الصفحة، حاول مجددًا', 'Could not open page. Retry'), () => setRetry(v => v + 1))}{pageUri && (Platform.OS === 'web' ? <Image source={{ uri: pageUri }} resizeMode="contain" style={[styles.image, { transform: [{ scale: zoom }] }]} /> : <WebView key={`${data.page}-${zoom}`} originWhitelist={['*']} source={{ html: mushafHtml(pageUri, zoom) }} onMessage={event => { if (event.nativeEvent.data === 'next') go(data.page + 1); else if (event.nativeEvent.data === 'prev') go(data.page - 1); }} style={styles.image} />)}</View>
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: '#FCFBF7' }, dashboard: { padding: 20, gap: 14, paddingBottom: 40 }, title: { fontSize: 23, fontWeight: '700', color: '#40362C', textAlign: 'center' }, sub: { color: '#796F65', lineHeight: 22, textAlign: 'center' }, note: { padding: 24, color: '#796F65' }, card: { backgroundColor: 'white', borderRadius: 20, borderWidth: 1, borderColor: '#EFE9DC', padding: 18, gap: 9 }, heading: { fontSize: 16, fontWeight: '600', color: '#40362C' }, big: { fontSize: 30, fontWeight: '700', color: '#D76743' }, green: { color: '#34775C' }, warning: { color: '#B04B2D', fontSize: 12 }, row: { minHeight: 42, flexDirection: 'row', alignItems: 'center', gap: 8, borderBottomWidth: .5, borderBottomColor: '#EAE5DD' }, button: { minHeight: 44, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 13, backgroundColor: '#F0F5EE', alignItems: 'center', justifyContent: 'center' }, primary: { backgroundColor: '#D76743' }, buttonText: { fontSize: 13, fontWeight: '600', color: '#B04B2D' }, white: { color: 'white' }, reader: { flex: 1 }, toolbar: { minHeight: 49, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', backgroundColor: 'white' }, ayahStrip: { flexGrow: 0, maxHeight: 48, backgroundColor: '#F9F7F2' }, ayahStripInner: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10 }, ayahHint: { color: '#796F65', fontSize: 11 }, ayahChip: { borderRadius: 9, paddingHorizontal: 9, paddingVertical: 5, backgroundColor: '#E9F2E9' }, ayahChipSelected: { backgroundColor: '#B9DFC9' }, ayahChipText: { color: '#295C48', fontSize: 12 }, page: { flex: 1, justifyContent: 'center', overflow: 'hidden', backgroundColor: 'white' }, image: { width: '100%', height: '100%' }, nav: { flexDirection: 'row', justifyContent: 'space-around', gap: 6, paddingVertical: 6, backgroundColor: 'white' }, input: { minWidth: 110, borderWidth: 1, borderColor: '#EAE5DD', borderRadius: 10, textAlign: 'center' },
});
