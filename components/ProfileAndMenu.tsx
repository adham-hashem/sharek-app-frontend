...
import {
  ...
  Alert, Modal, ActivityIndicator, TextInput, Platform, Pressable, Animated,
} from 'react-native';
...
export default function ProfileAndMenu({ profileView = false, initialSection, drawerOnly = false, drawerVisible = false, onDrawerClose }: { profileView?: boolean; initialSection?: string; drawerOnly?: boolean; drawerVisible?: boolean; onDrawerClose?: () => void }) {
  ...
  const visibleDrawer = drawerOnly ? drawerVisible : drawerOpen;

  useEffect(() => {
    if (!visibleDrawer) return;
    drawerSlide.setValue(rtl ? 380 : -380);
    Animated.timing(drawerSlide, { toValue: 0, duration: 220, useNativeDriver: true }).start();
  }, [visibleDrawer, rtl, drawerSlide]);

  const closeDrawer = () => drawerOnly ? onDrawerClose?.() : setDrawerOpen(false);
  ...
  const MenuItem = ({
    ...
    <TouchableOpacity style={[styles.menuItem, { flexDirection: rtl ? 'row-reverse' : 'row' }]} onPress={() => { closeDrawer(); onPress(); }} activeOpacity={0.7}>
  ...
  return (
    <>
      {profileView && <ScrollView ref={profileScrollRef} style={styles.container} contentContainerStyle={{ paddingBottom: spacing.xxl + 88 }}>
      ...
      <Modal visible={visibleDrawer} transparent animationType="fade" onRequestClose={closeDrawer}>
        <View style={styles.drawerOverlay}>
          <Pressable style={styles.drawerBackdrop} onPress={closeDrawer} accessibilityLabel={t('back')} />
          <Animated.View style={[styles.drawerPanel, rtl ? { right: 0 } : { left: 0 }, { transform: [{ translateX: drawerSlide }] }]}>
          <SafeAreaView style={{ flex: 1 }} edges={['top', 'bottom']}>
            <View style={[styles.drawerTop, { flexDirection: rtl ? 'row-reverse' : 'row' }]}>
              ...
              <TouchableOpacity onPress={closeDrawer} style={styles.drawerClose} accessibilityRole="button" accessibilityLabel={t('back')}>
